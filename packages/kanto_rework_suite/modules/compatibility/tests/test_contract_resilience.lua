...
local voxel={
  ...
  exports={lib={require=function(name)
    ...
    if name=="OverworldBattle" then return {battle=function() return stagedBattle end} end
...
local voxelAdapter
for _,a in ipairs(registeredAdapters) do if a.version=="1.8.5" then voxelAdapter=a end end
assert(voxelAdapter and voxelAdapter.version=="1.8.5","registered Voxel family adapter follows the live 1.8.5 release")
assert(voxelAdapter.match({id="BATTLE_ART_VOXEL_FORK",version="1.8.5"}),"family adapter matches live id/version without requiring a frozen name")
assert(mod.exports.release=="0.4.20","release export updated")
assert(mod.exports.rules.battleArtVoxel.version=="1.8.5" and mod.exports.rules.battleArtVoxel.latestAudited=="1.9.2","audit rule records live + latest audited release")
...
assert(sawOptions and sawStep,"1.8.5 stable contracts install both options and input bridges")
...
local battleA,battleB={},{}
local noSession=mod.exports.battleVisualPolicy(nil,battleA)
assert(noSession.backgroundOwner=="krs" and noSession.provider=="kanto_rework_ui.battle_background" and noSession.exclusive==false,
  "Voxel installed but no staged 3D battle keeps KRS BattleBackGround ownership")
stagedBattle=battleA
local liveSession=mod.exports.battleVisualPolicy(nil,battleA)
assert(liveSession.backgroundOwner=="voxel" and liveSession.provider=="battle_art_voxel.background" and liveSession.exclusive==true,
  "the exact live Voxel OverworldBattle session exclusively owns the battle environment")
local otherBattle=mod.exports.battleVisualPolicy(nil,battleB)
assert(otherBattle.backgroundOwner=="krs" and otherBattle.provider=="kanto_rework_ui.battle_background",
  "a different BattleState cannot inherit Voxel background ownership")
stagedBattle=nil
local ended=mod.exports.battleVisualPolicy(nil,battleA)
assert(ended.backgroundOwner=="krs" and ended.exclusive==false,
  "ending the Voxel session restores KRS background ownership without persistence")
local selfAdapter
...
local game={mods={optionSchemas={
  ...
  SFXMusicReplacementMod={
    {key="soundtrack",label="SOUNDTRACK"},
    {key="sfx_pack",label="SOUND EFFECTS"},
    {key="stereo",label="STEREO SOUND"},
  },
  kanto_rework_core={{key="accessibility",label="COLOR ACCESSIBILITY"}},
}},data={render_pipelines={
  _owners={kanto_rework_accessibility="kanto_rework_core"},
  kanto_rework_accessibility={label="COLOR ACCESSIBILITY"},
}}}
...
local filtered=filter(function(_,rows) return rows end,game,{
  ...
  {id="foo",label="FOO"},
  -- SFXMusicReplacementMod 2.0.0 re-exposes its schema with custom ids.
  {id="mod_soundtrack",label="SOUNDTRACK"},
  {id="mod_sfx_pack",label="SOUND EFFECTS"},
  {id="mod_stereo",label="STEREO SOUND"},
  -- KRS accessibility is both a standard mod schema option and a render pipeline.
  {id="pipeline:kanto_rework_accessibility",label="COLOR ACCESSIBILITY"},
  {id="pipeline:voxel",label="VOXEL"},
})
assert(#filtered==1 and filtered[1].id=="textSpeed","standard/custom mod options plus equivalent mod-owned pipelines are removed from global Options by ownership")
...
love=oldLove
print("Compatibility contract-family routing survives Battle Art Voxel Fork 1.8.5")
